...
if(version){
  ...
  ok('release standard version consistency',version.release_standard_version==='V4.2',version.release_standard_version||'missing');
  ok('release package id consistency',version.release_package_id==='2026.09.23-r3-v4.2-standard',version.release_package_id||'missing');
...
ok('release standard V4.2 marker',standard.includes('Standard version: V4.2'));
ok('release standard canonical V4.2 SHA-256',sha(read('docs/RELEASE_STANDARD.md'))==='f3b1358844a9f04da5ea8bfd6fe87b3051bd052e753bd8451991b39f894972b2');
...
// V4.2 sections 70-72: deterministic line-ending policy for hash-covered text files.
const attrs=exists('.gitattributes')?text('.gitattributes'):'';
ok('line-ending policy: default text eol=lf',/^\* text=auto eol=lf$/m.test(attrs));
for(const p of ['index.html','sPg_Material_Radar_Signature_Crafting_Quality.html','release/*.html','test-artifacts/**'])
  ok(`line-ending policy: byte-exact ${p} -text`,attrs.split(/\r?\n/).includes(`${p} -text`));
let crlf=[];
for(const f of (inventory?.files||[])){const a=abs(f.path);if(fs.existsSync(a)){const b=fs.readFileSync(a);if(!b.includes(0)&&b.includes(13))crlf.push(f.path)}}
ok('no CRLF in inventoried text files',crlf.length===0,crlf.slice(0,10).join(', '));
