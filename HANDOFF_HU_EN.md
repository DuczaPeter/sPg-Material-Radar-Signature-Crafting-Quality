...
- `Csak ezeket a radaron` esetén a radar a kiválasztott anyagokra szűkül, és ez felülírja a `Csak crafting anyagok` láthatósági szűrőt, hogy például Aluminum is megjelenhessen.
...
- `Show only these on radar` filters the radar to the selected materials and intentionally overrides the `Crafting materials only` visibility filter so materials such as Aluminum can remain visible.
