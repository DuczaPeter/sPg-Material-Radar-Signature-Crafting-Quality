...
## Mobil radar regression gate / Mobile radar regression gate

### HU
- 2026-08-30 mobil javítás: **900 px és alatta** a kijelölt/kinyitott sorok és a Target Mining becsukott radar-fókusz sorai külön, kompakt `--chip-*-mobile-safe` kapszulaméretet használnak.
- A mobil kapszulaszélesség a tényleges fix radaroszlop-távolság alatt marad; cél: a kapszulák ne csússzanak egymásba keskeny kijelzőn.
- **Desktopon 900 px felett ezt a mobil override-ot tilos alkalmazni.** A Target Mining becsukott fókuszsor továbbra is a nagy `--chip-*-selected` méretet használja, a kinyitott sor pedig változatlanul a meglévő selected megjelenést kapja.
- A javítás kizárólag radar-kapszula CSS/`layoutRadar()` geometriát módosít. Nem módosíthatja a MATERIALS, COMPONENT_REGISTRY, Quality-location, refinery, Target Mining találati logika, HU/EN, Aaron Halo vagy blueprint kapcsolatokat.
- Regression minimum: 360 / 390 / 430 / 560 / 768 / 900 px mobil/tablet szélességnél ne legyen kapszulaátfedés; 1024 px+ desktop nézetnél a korábbi nagy Target Mining becsukott kapszulák maradjanak meg.

### EN
- 2026-08-30 mobile fix: at **900 px and below**, selected/expanded rows and collapsed Target Mining radar-focus rows use a separate compact `--chip-*-mobile-safe` capsule envelope.
- Mobile capsule width is capped below the effective fixed radar-column spacing to prevent adjacent capsules from overlapping on narrow screens.
- **Above 900 px the mobile override must not apply.** Collapsed Target Mining focus rows keep the large `--chip-*-selected` sizing on desktop, while expanded rows retain the existing selected presentation.
- This fix is restricted to radar capsule CSS / `layoutRadar()` geometry. It must not change MATERIALS, COMPONENT_REGISTRY, Quality-location data, refinery data, Target Mining result logic, HU/EN behavior, Aaron Halo helper, or blueprint relationships.
- Minimum regression widths: 360 / 390 / 430 / 560 / 768 / 900 px without capsule overlap; 1024 px+ must preserve the large desktop collapsed Target Mining capsules.
