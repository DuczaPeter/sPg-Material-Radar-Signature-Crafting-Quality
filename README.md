...
**HU:** A felső `Célzott bányászat` mód bekapcsolása után az anyagokat közvetlenül a nagy radarlistában, az anyagnév melletti jelölőnégyzettel lehet ki- és bekapcsolni. Nem kell külön legördülőből hozzáadni őket. A rendszer a beágyazott SCMDB Quality snapshot alapján megkeresi azokat a top Quality helyeket, ahol legalább két kiválasztott anyag közösen szerepel. A találatok lefedettség szerint rendeződnek. A `Csak ezeket a radaron` gomb a radarlistát ideiglenesen csak a kiválasztott anyagokra szűri; desktopon a becsukott kiválasztott sorok radar-számai ugyanolyan nagyok maradnak, mint kinyitva, mobilon pedig külön kompakt méretezés védi őket az átfedéstől. Ugyanazzal a gombbal a teljes radar visszaállítható. Ez a funkció nem talál ki új lelőhelyeket és nem használ külön runtime API-t.

**EN:** After enabling `Target Mining`, materials are selected directly in the main radar list using the checkbox next to each material name. No separate material picker is required. Using the embedded SCMDB Quality snapshot, it finds top-Quality locations shared by at least two selected materials and ranks them by material coverage. `Show only these on radar` temporarily filters the radar to the selected materials; on desktop, collapsed selected rows keep the same large radar-number sizing as expanded rows, while mobile uses a separate compact size to prevent overlap. The same button restores the full radar. The feature does not invent additional locations and adds no runtime API dependency.
...
<!-- QDRIVE_AUDIT_410 -->
## Quantum Drive registry audit / Quantum Drive registry audit

**HU:** A Quantum Drive registry a `4.10.0-LIVE.12519617` StarCitizenWiki LIVE dataset alapján teljes auditot kapott. A korábbi részleges lista helyett **57 creation blueprint** van beágyazva S1–S4 méretig. Az Agni helyesen Agricium Case kapcsolatot kapott. A coverage audit két korábban hiányzó material-slot kapcsolatot is javított: `Corundum → Quantum Drive → Containment Matrix` és `Titanium → Quantum Drive → Case`. Részletek: `QUANTUM_DRIVE_AUDIT_4.10.md`.

**EN:** The Quantum Drive registry was fully audited against StarCitizenWiki LIVE `4.10.0-LIVE.12519617`. The previous partial list has been replaced with **57 embedded creation blueprints** covering S1–S4. Agni now correctly appears under Agricium Case. The coverage audit also fixed two missing material-slot mappings: `Corundum → Quantum Drive → Containment Matrix` and `Titanium → Quantum Drive → Case`. Details: `QUANTUM_DRIVE_AUDIT_4.10.md`.

### Mobile radar regression note

**HU:** 900 px és alatta a radar kapszulák külön mobil-safe méretezést használnak, hogy a kiválasztott/kinyitott és Target Mining sorok számai ne csússzanak egymásba. 900 px felett a desktop Target Mining becsukott sorok nagy radar-kapszulái változatlanul megmaradnak.

**EN:** At 900 px and below, radar capsules use a separate mobile-safe size so selected/expanded and Target Mining rows do not overlap. Above 900 px, the large desktop collapsed Target Mining capsules remain unchanged.
