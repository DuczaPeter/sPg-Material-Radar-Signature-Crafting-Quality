...
The persistent release authority is [`RELEASE_STANDARD.md`](RELEASE_STANDARD.md), currently pinned to **V4.2**. The current release-specific gate contract is [`RELEASE_CONTRACT.md`](RELEASE_CONTRACT.md).
...
9. Build the ZIP from repository-root contents with no outer nesting directory, including dot-prefixed files.
10. Hash the final ZIP separately.
11. Publish with git or GitHub Desktop, not the web uploader, so `.github/`, `.gitignore` and `.gitattributes` are included.
12. Run the same gate on a fresh clone of the published repository (`REPOSITORY PUBLICATION = MANUAL BY USER`).
